...
import type { Location, Match, ContextRouter, RouterHistory } from 'react-router';
...
<Route path="/" children={({ match }) => <div className={match ? 'active' : ''} />} />;
<Route>
  <div>children</div>
</Route>;
...
const Routed1: React$ComponentType<{| someProp: string |}> = withRouter(Unrouted);
...
const Routed2: React$ComponentType<{| someProp2: string |}> = withRouter(Unrouted2);
