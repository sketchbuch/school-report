...
describe('Reducer: Classes', () => {
  ...
  test('REPLACE_DATA should return the initial state if payload has no classes array', () => {
    ...
    expect(reducer(INITIAL_STATE, { type: REPLACE_DATA, payload: {classes: 'wrong'} })).toEqual(INITIAL_STATE);
  });
  ...
  test('DATA_LOADED should return the initial state if payload has no classes array', () => {
    ...
    expect(reducer(INITIAL_STATE, { type: DATA_LOADED, payload: {classes: 'wrong'} })).toEqual(INITIAL_STATE);
  });
  ...
  test('UPDATE_CLASS should update existing categories', () => {
    ...
    const reducerResult = reducer(INITIAL_STATE, { type: UPDATE_CLASS, payload: NEW_CAT });
  ...
  test('DELETE_CLASS should delete the category if the ID is found', () => {
    ...
    const reducerResult = reducer(INITIAL_STATE_DEL, { type: DELETE_CLASS, payload: INITIAL_STATE_DEL[1] });
