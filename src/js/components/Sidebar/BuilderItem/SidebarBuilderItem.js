  ...
  render() {
    ...
    return (
      <li className={classes} title={classRec.getTooltip()}>
        <SidebarInner onClick={this.handleExpandClick} description={classRec.getDescription()} label={classRec.getLabel()} icon={classRec.getIcon()}>
          <span className="SidebarItem_expander" title={text('Expand', 'Actions')}><Icon type={ ICON_EXPAND } /></span>
          <span className="SidebarItem_contractor" title={text('Contract', 'Actions')}><Icon type={ ICON_CONTRACT } /></span>
        </SidebarInner>
        {this.state.open &&
