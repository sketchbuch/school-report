...
export class SettingsLayout extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    if (window.location.pathname === ROUTE_SETTINGS) setTitle(text('WinTitle', 'Settings'));
    ...
    if (this.state.error) {
      ...
      this.props.dispatch(settingsActions.update(this.state.settings, this.dataSaved));
  ...
  */
  dataSaved(ioResult: Object) {
    if (ioResult.success === true) {
      this.props.dispatch(languageActions.change(this.state.settings.language, this.languageChanged));
    } else {
      this.setState({
        error: true,
        saving: false,
  ...
  */
  languageChanged() {
    const langObject = this.props.languages.find( lang => lang.key === this.state.settings.language);
    const langText = (langObject !== undefined) ? `${text('LanguageChanged', 'Settings')} ${langObject.label} (${langObject.key})` : '';
